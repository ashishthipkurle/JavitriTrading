...
export default async function AdminClientInvestmentForm({ params }: { params: { id: string } }) {
  ...
  if (!client.kyc) {
    return (
      <div className="flex flex-col items-center justify-center py-20 gap-4">
        ...
        <Link href="/admin/users" className="bg-primary text-on-primary px-4 py-2 rounded font-bold hover:brightness-90">
          Back to Users
        </Link>
      </div>
    );
    ...
    <div className="flex flex-col gap-6 max-w-4xl mx-auto py-8">
      <div className="no-print flex justify-between items-center bg-surface-container p-6 rounded-xl border border-outline-variant">
        ...
        <div className="flex gap-4">
          <Link href="/admin/users" className="border border-outline-variant px-4 py-2 rounded font-bold hover:bg-surface-container-low transition">
            Back
          </Link>
          <ClientSidePrint />
        </div>
